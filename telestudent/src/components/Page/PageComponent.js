    ...
    return (
      <div className="demo-big-content">
        <Layout fixedHeader>
          ...
          <Drawer fixeddrawer="fixedDrawer" title={title} open={this.props.left}>
            <Navigation>
              {items.map((item,index) =>
                <MenuItem key={index} id={item.page} onClick={this.props.handleIndex} title={item.blocks}>{item.blocks}</MenuItem>
              )}
            </Navigation>
          </Drawer>
          <Content >
